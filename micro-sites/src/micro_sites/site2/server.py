...
from bareutils import text_response, response_code
...
def load_config():
    initialise_types()
    with open(pkg_resources.resource_filename('micro_sites.site2', 'config.yaml'), 'rt') as fp:
        return edict(yaml.load(fp, Loader=yaml.FullLoader))


# noinspection PyUnusedLocal
async def get_info(scope, info, matches, content):
    return text_response(response_code.OK, None, 'site2')
