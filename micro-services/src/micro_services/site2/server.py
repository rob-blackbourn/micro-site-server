...
async def get_info1(scope, info, matches, content):
    try:
        logger.info('GET site2 info1')
        return json_response(response_code.OK, None, {'message': 'Site 2 Info 1'})
    except:
...
async def get_info2(scope, info, matches, content):
    try:
        logger.info('GET site2 info2')
        return json_response(response_code.OK, None, {'message': 'Site 2 Info 2'})
    except:
        logger.exception('Failed to get info')
        return response_code.INTERNAL_SERVER_ERROR, None, None
